...
### Sprint 10 — 学習アプリ「アジャイル・スクラム編」

合意: [brainstorm.md](brainstorm.md) 2026-09-26、[ADR 0010](adr/0010-agile-course.md)。API の実装と API 編の教材は変えない。

含むもの:
- `index.html` のコース切り替え（REST API / アジャイル・スクラム）。URL・章の解放・進捗・苦手・記録をコースごとに分ける
- 章の型に `wf:` 欄（ウォーターフォールではこうだった）を追加。アジャイル編では必須
- `chapters/agile/outline.js` ＋ 1〜9 章（各章 15 問以上、全節に図、実物がある節にはリンクと「確認すること」）
- `LearningLinksTest` を `chapters/agile/` にも広げる。`AUTHORING.md` にコースと `wf:` を追記

受け入れ基準:
確かめたものにチェックを付ける（Sprint 10 から。確かめ方は [progress.md](progress.md) の Sprint 10）。

- [x] ヘッダーでコースを切り替えられ、アジャイル編の 9 章が開ける。章データの自己検査に問題が出ない。各章 15 問以上
- [x] アジャイル編は 1 章から独立に解放される（API 編の合否に影響されず、影響もしない）
- [x] API 編の既存の進捗（Sprint 9 の保存データ・書き出したファイル）がそのまま読め、表示が変わらない
- [x] 苦手な問題・学習の記録はコースごとに表示される。書き出し・読み込みで両コースの進捗が復元される
- [x] アジャイル編の全章に「ウォーターフォールではこうだった」欄がある
- [x] `./mvnw verify` がグリーン（アジャイル編のリンクも検査され、1 つ壊すと赤くなる）
- [ ] 390px 幅（スマホ）でもヘッダーと本文が読める（ブラウザでの確認は利用者に依頼）
