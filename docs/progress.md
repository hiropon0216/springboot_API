...
| ドキュメント | `brainstorm` / `spec` / `README` / `CLAUDE.md` / ADR 0006 / 本ファイル |
...
- `git tag sprint-5` と `docs/learning/` の計算 API 版は未着手（任意）。

### 引き渡し事項

- **コミットはまだしていない。** working tree に大量の削除・追加・改名がある。`main` で作業したので、
  コミットするならブランチを切ってから。
- `docs/learning/sprint-0.md` `sprint-1.md`、`docs/adr/0003`（PostgreSQL）、`glossary.md` の
  Sprint 0〜1 節は現行コードに対応しない。学習記録として残置。
