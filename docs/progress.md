...
## Sprint 10 — 学習アプリ「アジャイル・スクラム編」✅ 実装完了（2026-09-26）

学習アプリに 2 つ目のコース「アジャイル・スクラム編」（9 章）を足した。合意は brainstorm 2026-09-26、
決定は [ADR 0010](adr/0010-agile-course.md)。API の実装と REST API 編の本文・問題は変えていない。

### やったこと

| 分類 | 内容 |
|---|---|
| コース切り替え | `index.html` のヘッダーに REST API / アジャイル・スクラム の切り替え。URL は API 編が従来どおり `#ch/3`、アジャイル編が `#agile/ch/3`・ホーム `#agile`。目次・ホーム・タイトル・種別バッジ（実装 → 実物）・「動かして観察」→「やってみる」をコースごとに |
| 保存データ | 形は Sprint 9 のまま。アジャイル編の鍵だけ `agile:` を付ける（`passed["agile:3"]`、問題 id `agile:3:…`、記録に `course`）。**Sprint 9 の保存データ・書き出しファイルはそのまま読める**（`course` の無い記録は API 編として扱う）|
| 章の型 | `wf:`（ウォーターフォールではこうだった）を追加。アジャイル編では必須（無ければ自己検査で警告）|
| 章 | `chapters/agile/outline.js` ＋ 1〜9 章。**144 問**（各章 16 問）・図 89 個（ステッパー 1・インライン SVG のバーンアップ 1 を含む）・実物へのリンク 28 個 |
| 実物 | spec（受け入れ基準・今後の候補・Sprint 節）、CLAUDE.md（DoD・役割）、progress（検証不能・見直し）、ADR 0006 / 0008、learning/sprint-9.md、ci.yml、brainstorm |
| リンク検査 | `LearningLinksTest` が `chapters/agile/` も対象に（リンク・行番号の一覧・`<script>` の読み込み漏れ）|
| 文書 | ADR 0010、spec の Sprint 10、AUTHORING.md（コース・`wf:`・インライン SVG・選択肢の長さ）、README、CLAUDE.md |

### 検証結果

`./mvnw verify` グリーン（64 tests）。Node の簡易 DOM で `index.html` の実物のスクリプトを動かして確認（すべて OK）:

| 受け入れ基準（spec.md Sprint 10）| 結果 |
|---|---|
| コースを切り替えられ、アジャイル編 9 章が開ける。自己検査に問題なし。各章 15 問以上 | ✅ 全 9 章の本文・テストを描画し undefined / NaN なし、全節に図、実物の章は refs のある節に checks。REST API 編も 26 章・424 問のまま描画できる |
| アジャイル編は独立に解放される | ✅ API 編 1・2 章合格の保存データでアジャイル編は 1 章だけ開いている。アジャイル編 1 章に合格 → 2 章が開き、API 編の合否は不変 |
| Sprint 9 の保存データがそのまま読める | ✅ Sprint 9 の形（鍵が章番号だけ・記録に course 無し）で API 編の合格・記録がそのまま表示 |
| 苦手・記録はコースごと、書き出し・読み込みで両コース復元 | ✅ アジャイル編で 3 問間違え → アジャイル編ホームにだけ苦手 3 問、復習は 3 問。書き出し → 空の状態に読み込みで両コースの合否が戻る。壊れた JSON は状態不変 |
| 全章に「ウォーターフォールではこうだった」 | ✅ 9 章すべて |
| verify グリーン、アジャイル編のリンクを壊すと赤 | ✅ ch04 の目印を 1 つ壊すと `LearningLinksTest` が 2 件失敗（リンク切れ・一覧の古さ）→ 戻して緑 |
| 390px 幅で読める | ⚠ 未確認（下記）|

内容の事実確認: 本文に書いたこのリポジトリの事実は実物で確かめた（`git tag -l "sprint-*"` は 0・1・7・8・9 のみ、
Sprint 7〜9 のコミット日はすべて 2026-09-26、spec.md のチェック欄は（このスプリントで付けるまで）`[x]` が 0 件、`docs/feedback/` は存在しない、
ci.yml は全ブランチの push と main への PR で verify、ADR 0008 の要望の文言）。
ADR 0008 の要望を言い換えて書いていた箇所（5 章）は、実際の文言に直した。
正解の選択肢だけが極端に長い問題 5 問は、誤りの選択肢を同じくらいの具体さに書き直した。

### 総点検（push 前）

アジャイル編 9 章を通しで読み、事実・日本語・表記を点検した。事実は一次情報と突き合わせた
（[スクラムガイド 2020 年版](https://scrumguides.org/scrum-guide.html)、
[IPA アジャイル開発版モデル契約](https://www.ipa.go.jp/digital/model/agile20200331.html)、
厚生労働省の疑義応答集（第 3 集、2021 年 9 月））。直したもの:

| 種類 | 内容 |
|---|---|
| 事実 | 4 章：レビューの出力を「見直されたプロダクトバックログ」と言い切っていた → ガイドは「調整されることもある」。デイリーの 3 つの質問を「2020 年版で必須ではなくなった」→「以前の版に例として載っていた」。3 章：SM は障害物を「取り除く」→「取り除かれるよう働きかける」、スプリントゴールは SM も含むチーム全員で決める。2 章：「これがスクラムの全部」→ 価値基準・経験主義も土台にある。バーンダウンは「ガイドに無い」→「決まりではない（例として名前が出る）」。1 章：原則 11 の要約が不完全だった |
| 事実（補足） | 8 章：疑義応答集の要点「発注側と開発者が直接やり取りしても、開発者が自律的に判断しているなら偽装請負にあたらない」を追加。IPA の公開時期を 2020 年 3 月と明記 |
| 数字の食い違い | 7 章：ベロシティの表の幅（18〜22）と、予測の計算・問題（18〜24）が食い違っていた → 表とバーンアップの数字を 18〜24 に合わせた。120 ÷ 18 の切り上げを明記 |
| 推測の言い切り | 2 章：2 週間スプリントでのイベントの長さを「〜が多い」と書いていた → 「長さに比例させた場合の目安」に |
| 日本語 | ユーザーストーリーの例文の区切り、「完成の定義の確認が最後に固まった」などの不自然な文、「価値はゼロ」などの強すぎる言い方 |
| 表記ゆれ | 曖昧 → あいまい（REST API 編 10 章も）、揃う → そろう、溜まる → たまる、出来上がる → できあがる（REST API 編 22 章も）、ブレ → ぶれ、index → インデックス |

全 35 章（両コース）の描画結果を機械的にも検査した：1,043 か所・15,441 個のタグで、閉じ忘れ・対応の誤り・エスケープ漏れ・
句読点の重複は無い。HTML の書き方は `<br>` と `<b>` にそろっている（`<br/>` `<strong>` などは無い）。

### 検証不能・未実施

- **ブラウザでの画面確認は未実施**（この環境にブラウザ操作の手段が無い）。ヘッダーのコース切り替えの見た目、
  390px 幅での折り返し、7 章のバーンアップ（SVG）のダークモードでの見え方、2 章のステッパーの操作感は利用者の確認待ち。
- 契約（民法の条文）は考え方の説明にとどめ、条文そのものは確認していない。

### 引き渡し事項

- アジャイル編 9 章で「守れていないこと」として、spec のチェック欄が空・sprint-2〜6 のタグが無い・`docs/feedback/` が無い、を挙げた。
  直すかどうか（チェックを付ける／タグを打つ／CLAUDE.md の表から feedback を外す）は次のスプリントで利用者が決める。
  このスプリントの受け入れ基準は、確かめたものにチェックを付けた。
- 保存データの鍵の形（API 編は章番号だけ、ほかは `コース:章番号`）を変えると、利用者の進捗が読めなくなる。変えるなら読み替えを入れる。
- `git tag sprint-10` とコミット・push は 2026-09-27 に実施（総点検の後）。
